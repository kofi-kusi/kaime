      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white p-6 rounded shadow mb-6 border">
          <div className="grid grid-cols-2 gap-4">
            ...
            <input required placeholder="First Name" className="border p-2 rounded" 
              onChange={e => setFormData({...formData, name: e.target.value})} />
            <input required placeholder="Surname" className="border p-2 rounded" 
              onChange={e => setFormData({...formData, surname: e.target.value})} />
            <input required placeholder="Other Names" className="border p-2 rounded" 
              onChange={e => setFormData({...formData, other_names: e.target.value})} />
            <input required placeholder="Program" className="border p-2 rounded" 
          ...
          <tbody>
            {subscribers.map((sub) => (
              <tr key={sub.email} className="border-b">
                ...
                <td className="p-3">
                  <button onClick={() => handleDelete(sub.email)} className="text-red-600 hover:underline">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
